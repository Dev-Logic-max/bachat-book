 ...
 *
 * ---------------------------------------------------------------------------
 * PORTED VERBATIM from `web/src/lib/modules.ts`. Refresh it by copying that file
 * over this one; everything mobile-specific lives below the `MOBILE ROUTES`
 * banner at the bottom, so a re-copy only costs you that block.
 *
 * `href` on each module is the WEB route. Native routes are a separate map —
 * editing the hrefs in place would be undone by the next copy, and the two route
 * spaces genuinely differ (the phone has five tabs and a More sheet where the
 * web has a 248px rail).
 * ---------------------------------------------------------------------------
 */
...
// ===========================================================================
// MOBILE ROUTES — everything below this line is native-only.
// ===========================================================================

/**
 * Native route per module key. A key absent from this map has no screen on the
 * phone yet.
 *
 * That absence is rendered, not hidden: the More grid shows the module greyed
 * with "on the web" beside it. A module that silently vanishes from the phone
 * reads as a missing feature, and the user has no way to learn it exists at all.
 */
export const MOBILE_ROUTES: Record<string, string> = {
  dashboard: "/",
  entries: "/entries",
  accounts: "/accounts",
...
/** The native route for a module, or null when it is web-only for now. */
export function moduleRoute(key: string): string | null {
  return MOBILE_ROUTES[key] ?? null;
}

/**
 * The four destination tabs. The fifth slot in the bar is the centre ADD button,
 * which is an action and not a route.
 *
 * Five tabs with the centre as an action is the same conclusion the web bottom
 * nav reached, for the same reason: logging money should not require navigating
 * to a page in order to press a button on it.
 */
export const TAB_KEYS = ["dashboard", "entries", "accounts", "more"] as const;
