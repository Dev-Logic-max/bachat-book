...
- **A deep link into Entries needs the MONTH as well as the id.** The list opens on the current month, so `?entry=<id>` alone lands on a page that does not contain the row. The task card's receipt chip carries `?month=YYYY-MM&entry=<id>`, which is why the tasks page fetches the settled transactions rather than trusting the task's own `completed_at`.
